...
async def get_pokemons(service: service_dependency, db: db_dependency, user: auth_dependency):
...
async def update_pokemon(pokemon_id: str, pokemon: PokemonRegisterDto, service: service_dependency, db: db_dependency, user: auth_dependency):
    pokemon = await service.update_pokemon(pokemon_id, pokemon, db)
    return pokemon

@router.delete("/{pokemon_id}", status_code=204)
async def delete_pokemon(pokemon_id: str, service: service_dependency, db: db_dependency, user: auth_dependency):
    await service.delete_pokemon(pokemon_id, db)

@router.delete("", status_code=204)
async def delete_all_pokemons(service: service_dependency, db: db_dependency, user: auth_dependency):
