import pytest
from ..mocks.mocks import mocked_abilities, mocked_create_ability_request, get_mocked_abilities
...
async def test_get_ability_by_id(test_client, fake_abilities_service):
    fake_abilities_service.get_ability_by_id.return_value = mocked_abilities[0]
    response = test_client.get("/abilities/1")
    assert response.status_code == 200
    assert response.json() == mocked_abilities[0]
...
async def test_create_ability(test_client, fake_abilities_service):
    fake_abilities_service.create_ability.return_value = mocked_abilities[0]
...
async def test_update_ability(test_client, fake_abilities_service):
    fake_abilities_service.update_ability.return_value = mocked_abilities[0]
