import pytest
from ..mocks.mocks import mocked_types, mocked_create_type_request_json, get_mocked_types
...
async def test_create_type(test_client, fake_types_service):
    ...
    response = test_client.post("/types", json=mocked_create_type_request_json)
    assert response.status_code == 200
    assert response.json() == mocked_types[0]
...
async def test_update_type(test_client, fake_types_service):
    ...
    response = test_client.put("/types/1", json=mocked_create_type_request_json)
    assert response.status_code == 200
    assert response.json() == mocked_types[0]
